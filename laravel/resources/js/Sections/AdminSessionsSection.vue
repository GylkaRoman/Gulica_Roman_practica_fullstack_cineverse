...
const saveSession = async () => {
    ...
    try {
        if (editingId.value) {
            await axios.put(`/api/sessions/${editingId.value}`, form.value, {
                headers: { Authorization: `Bearer ${token}` },
            });
        } else {
            await axios.post("/api/sessions", form.value, {
                headers: { Authorization: `Bearer ${token}` },
            });
        }
        ...
        <div class="w-full max-w-3xl bg-gray-950 p-8 rounded-2xl shadow-lg">
            ...
            <div class="grid grid-cols-2 gap-3 mb-6">
                ...
                <div>
                    <select
                        v-model="form.format"
                        class="w-full p-3 bg-gray-800 rounded-lg required"
                    >
            ...
            <button
                @click="saveSession"
                class="w-full bg-primary text-black font-bold py-3 rounded-lg mb-6"
            >
            ...
            <div
                v-for="s in sessions"
                :key="s.id"
                class="bg-gray-800 p-4 mb-3 rounded-lg flex justify-between"
            >
                <div>
                    ...
                    <p class="text-sm text-gray-300">
                        {{ s.date }} {{ s.time }} | {{ s.format }} |
                        {{ s.language }} | {{ s.base_price }} MDL
                    </p>
                </div>
